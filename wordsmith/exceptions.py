...
class NarrativeGenerateError(Exception):
    """The Wordsmith platform responded with an error code when attempting to
    generate narrative."""
    ...
    def __init__(self, response, data):
        ...
        self.data = data
        try:
            self.details = [str(e['detail'])
                            for e in response.json()['errors']]
            self._details_reported = True
        except KeyError:
            self.details =\
                  ['Wordsmith reported an error but no details were provided.']
        ...
        self.msg = '\nError generating narrative.' + \
                   ...
                   '\nNumber of errors reported by Wordsmith: {}'.\
                   format(len(self.details) if self._details_reported else 0)
